...
for (const engine of engines) {
  ...
  try {
    ...
    assert.match(await page.locator('[data-action="continue"]').innerText(), /S03/, 'Saved sector boundary survives the update');
    ...
    console.log(JSON.stringify({ engine, reproducedWaitingWorker: true, updatedWhileOldTabOpen: true, autoRefreshed, ordinaryReloadFresh: true, savedProgressPreserved: true, cacheNames }));
  } catch (error) {
    failures++;
