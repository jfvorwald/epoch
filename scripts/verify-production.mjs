...
for (const engine of ['chrome', 'webkit']) {
  ...
  try {
    ...
    await page.goto(origin.url + '?playtest=1');
    ...
    assert.equal(await page.locator('.playtest-badge').count(), 0, 'Playtest query cannot enable preview mode in production');
    await page.locator('[data-action="hangar"]').click();
    await page.locator('[data-ship="manta"]').click();
    assert.equal(await page.locator('[aria-label="Manta parts collected"]').getAttribute('aria-valuenow'), '0', 'Production Manta must remain locked with no parts');
