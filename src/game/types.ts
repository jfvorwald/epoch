...
export type PickupKind = 'health' | 'rapid' | 'spread' | 'damage' | 'multishot' | 'weapon-pulse' | 'weapon-lance' | 'weapon-scatter' | 'ship-part';
export type FormationKind = 'chevron' | 'arc' | 'pincer' | 'column' | 'line';
export interface Wave { at: number; kind: EnemyKind; formation: FormationKind; count: number; hp: number; speed: number; hold: number; drop?: PickupKind; tier?: number; }
...
export interface SaveData { version: 1; bestScore: number; furthestSector: number; checkpoint: Checkpoint | null; unlockedTransmissions: string[]; preferences: Preferences; hangar: { selectedShip: ShipId; parts: number; loadouts: Record<ShipId, ShipLoadout> }; }
export interface HudState { level: number; score: number; hp: number; maxHp: number; shipId: ShipId; weapon: WeaponKind; multishot: number; projectiles: number; buffs: Partial<Record<PickupKind, number>>; progress: number; wave: number; totalWaves: number; bossHp?: number; bossMaxHp?: number; bossName?: string; }
